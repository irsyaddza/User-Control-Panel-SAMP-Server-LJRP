...
$userID = $_SESSION['user_id']; // This is already the ID from accounts table
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    elseif (!preg_match('/^\d{1,5}$/', $model)) {
        ...
        $stmt = $conn->prepare("SELECT ID FROM characters WHERE char_name = ?");
...
$stmt = $conn->prepare("SELECT c.*, a.Username FROM characters c 
